...
int store_add(store_t *st, session_t *s)
{
	...
	/* Vérifier doublons : si ID déjà présent, on détruit la session reçue */
	cur = st->head;
	while (cur) {
	...
	/* Créer le nœud pour l'insertion */
	n = node_create(s);
	if (!n) {
		/* node non créé => libérer la session pour éviter la fuite */
		session_destroy(s);
		return 0;
	}
...
int store_delete(store_t *st, const char *id, session_t **out)
{
	...
	while (cur) {
			...
			strcmp(cur->sess->id, id) == 0)
		{
			...
			/* Transfert de propriété si out != NULL */
			if (out) {
				*out = cur->sess;
			} else {
				/* Le store reste responsable de la destruction */
